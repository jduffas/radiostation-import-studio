...
//   0-5 s   instrumental  ← touche le tout début de la piste : DOIT être exclue (c'est le
//                           rôle de l'intro/des cue points, pas de la zone "jingle intérieur")
//   5-20 s  « voix »
//   20-24 s instrumental  ← seule zone attendue (vraiment intérieure à la piste)
//   24-32 s « voix »
'use strict';
...
(async () => {
  ...
  check('au moins 1 zone détectée', zones.length >= 1, String(zones.length));
  ...
  check('aucune zone ne touche le tout début ou la toute fin de la piste',
    zones.every(z => z.start_ms > 0 && z.end_ms < 32000), JSON.stringify(zones));
